...
import com.sumscope.cdhplus.realtime.quickfixj.callback.BestQuoteCallback;
 ...
 */
@Component
public class BestQuoteReceiver {

    private static final Logger LOGGER = LoggerFactory.getLogger(BestQuoteReceiver.class);

    @Autowired
        private BestQuoteCallback best_quote_callback;

    @Value("${best.quote.queue}")
    ...
    public void start(){
        ...
        try {
            receiver = ReceiverFactory.newReceiver(createOptions,best_quote_callback);
        } catch (Exception e) {
