...
import com.sumscope.cdhplus.realtime.quickfixj.callback.ProfparsedCallback;
 ...
 */
@Component
public class ProfparsedReceiver {

    private static final Logger LOGGER = LoggerFactory.getLogger(ProfparsedReceiver.class);

    @Autowired
    private ProfparsedCallback profparsed_callback;

    @Value("${profparsed.queue}")
    ...
    public void start(){
        ...
        try {
            receiver = ReceiverFactory.newReceiver(createOptions,profparsed_callback);
        } catch (Exception e) {
